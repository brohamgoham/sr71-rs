...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    let helium_key = env::var("HELIUM_API_KEY")
        .expect("MISSING HELIUM API KEY");
    ...
    let bloxk = rpc.get_latest_blockhash_with_commitment(CommitmentConfig::confirmed()).unwrap();
